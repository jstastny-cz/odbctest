...
class ResultsComparator(object):
    ...
    def compare_columns(self, ex_res, ac_res):
        ...
        if len(ex_res.columns) != len(ac_res.columns):
            ...
            for col1, col2 in [(ex_res.columns[i], ac_res.columns[i])
                               for i in range(0, len(ex_res.columns))]:
                if col1.name.lower() != col2.name.lower():
                    failures.append(
                        ComparationFailure(
                            "Compare error: column names differ.", col1.name,
                            col2.name, ac_res.query_name, ac_res.query))
                if col1.col_type != col2.col_type:
                    failures.append(
                        ComparationFailure(
                            "Compare error: column types differ.",
                            col1.col_type, col2.col_type, ac_res.query_name,
                            ac_res.query))
    ...
    def compare_rows(self, ex_res, ac_res):
        ...
        if len(ex_res.rows) != len(ac_res.rows):
            ...
            for row1, row2 in [(ex_rows[i], ac_rows[i])
                ...
                for ex_cell, ac_cell in [(row1[i], row2[i])
