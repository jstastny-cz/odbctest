...
class ODBCHelper:
    ...
    def execute_query(self, query):
        ...
        if stmt.get_type() not in ["INSERT", "DELETE", "UPDATE"
            ...
            if stmt.get_type() in ["DELETE"]:
                self.cursor.commit()
            self.rows = []
        result = map(lambda x:[data.encode('utf-8') if isinstance(data,unicode) else str(data) for data in [(x[cell] if not x[cell] is None else "null") for cell in range(0,len(x))]],[row for row in self.rows])
        return result
    ...
    def parse_columns_from_query(self, query):
        ...
        for token in stmt.tokens:
            ...
            if token.ttype == sqlparse.tokens.Keyword and str(
                    token).lower() == "fetch":
            ...
            if token.ttype == None and cast_type:
                columns.append(
                    Column(columns_retreived[num_of_cols],
                           "CAST(" + str(token) + ")",
                           self.decide_type_cast(query, num_of_cols)))
                cast_type = False
            elif token.ttype == sqlparse.tokens.Wildcard and str(token) == "*":
                for col in self.cursor.description:
                    columns.append(
                        Column(col[0], None,
                               self.get_type_of_column(None, col[0], query)))
                ...
                select_cols = re.findall(
                    "(" + match_function + "\s*(AS\s+\w+)*,?|(" +
                    match_fq_column_name + "\s*(AS\s+\w+)*,?)|(" +
                    match_aritmetic_ops + "))", str(token), re.IGNORECASE)
                for col in map(lambda x: x[0].strip(), select_cols):
                    if col.endswith(","):
                        col = col[:-1]
                    col = col.strip()
                    parts = col.split(".")
                    alias = parts[0].strip() if re.search(
                        "FROM\s+.*\s+AS\s+\w*", query,
                        re.IGNORECASE) and len(parts) == 2 else None
                    col_name = parts[-1].split(".")[-1].strip()
                    columns.append(
                        Column(columns_retreived[num_of_cols], col,
                               self.get_type_of_column(alias, col_name,
                                                       query)))
                    num_of_cols = len(columns)
            elif token.ttype == sqlparse.tokens.Keyword and str(
                    token).lower() == "cast":
                cast_type = True
        return columns
    ...
    def decide_type_cast(self, query, column_offset):
        ...
        lowest_idx = -1
        for token in stmt.tokens:
            if (token.ttype in [sqlparse.tokens.Keyword, None]
                    and lowest_idx < 0):
                lowest_idx = token
            if (token.ttype == sqlparse.tokens.Keyword
                    and str(token).lower() == "from"):
                highest_idx = token
    ...
    def decide_column_type(self, table, col_name):
        ...
        parsed_table = re.match(
            "\s*((((\w+)\.)*(\w+)\.)*(\"([\w\.]+)\"|\w+))\s*", table)
        if (parsed_table is None):
            return "undefined"
        table_qualified = [
            parsed_table.group(7) or parsed_table.group(6),
            parsed_table.group(5),
            parsed_table.group(4)
        ]
        ...
        if not table.lower().strip() in self.db_columns:
            self.db_columns[table.lower().strip()] = [
                x
                for x in self.cursor.columns(
                    table=result_table_tuple[0],
                    schema=result_table_tuple[1],
                    catalog=result_table_tuple[2])
            ]
        column_type_list = map(
            lambda x: x.type_name.lower(),
            filter(lambda y: y.column_name.lower() == col_name.lower(),
                   self.db_columns[table.lower().strip()]))
        column_type = column_type_list[
            0] if len(column_type_list) > 0 else "undefined"
        return column_type
    ...
    def get_table_tuple(self, table_qualified_tuple):
        ...
        for table_tuple in self.db_tables:
            ...
            if table_qualified_tuple[1] != None:
                result = result and (
                    table_qualified_tuple[1].lower() == table_tuple[1].lower())
            if table_qualified_tuple[2] != None:
                result = result and (
                    table_qualified_tuple[2].lower() == table_tuple[2].lower())
            if result:
    ...
    def get_type_of_column(self, alias, col_name, query):
        ...
        if (re.match("SELECT.*" +
                     ((re.escape(alias) + "\.")
                      if alias else "") + re.escape(col_name) + ".*FROM.*",
                     query, re.IGNORECASE)
                or re.match("SELECT\s+\*\s+FROM.*", query, re.IGNORECASE)):
            tables = self.get_queried_tables(query)
            table_array = self.search_queried_tables(alias, col_name, query,
                                                     tables)
...
#
    ...
    def search_queried_tables(self, alias, col_name, query, queried_tables):
        ...
        for table in queried_tables:
            if alias != None and table[0] != None and table[0].lower(
            ) == alias.lower():
                if isinstance(table[1], basestring):
                    found.append(table[1])
                else:
                    inner_parsed = re.split(".*\((.*)\).*", table[2],
                                            re.IGNORECASE)
                    inner_query = inner_parsed if len(
                        inner_parsed) < 3 else inner_parsed[
                            1:len(inner_parsed) - 1][0]
                    inner_split = re.split("\s*(\w*)\." + col_name,
                                           inner_query, re.IGNORECASE)
                    inner_alias = None if len(
                        inner_split) < 3 else inner_split[1]
                    found += self.search_queried_tables(
                        inner_alias, col_name, inner_query, table[1])
                break
            elif alias != None and not isinstance(table[1], basestring):
                found += self.search_queried_tables(alias, col_name, query,
                                                    table[1])
            elif alias == None and not isinstance(table[1], basestring):
                found += self.search_queried_tables(None, col_name, query,
                                                    table[1])
            elif alias == None:
                found.append(table[1])
...
#
    ...
    def queried_tables(self, query):
        ...
        if found:
            defined_tables.append((found.group(2), self.queried_tables(
        ...
        for token in stmt.tokens:
            ...
            if (token.ttype == sqlparse.tokens.Keyword
                    and token_string.lower() in [
                        "from", "inner join", "full outer join",
                        "right outer join", "left outer join", "union all"
                    ]):
                from_idxs.append(stmt.tokens.index(token))
            if (token.ttype == None
                    and (token_string.lower().startswith("select")
                         or token_string.lower().startswith("(select"))):
                ...
                child = children[0][1] if len(children) == 1 and isinstance(
                    children[0][1], basestring) else children
                defined_tables.append((table_alias, child, inner_select))
        top_level_from_idxs = filter(
            lambda x: x + 2 not in inner_selects_idxs,
            from_idxs)  # indexes of top-level 'FROM' query tokens
        # return tokens which follows 'FROM' clauses denoted by top_level_from_idxs
        with_possible_commas = filter(
            lambda x: (stmt.tokens.index(x) - 2) in top_level_from_idxs,
            stmt.tokens)
        without_commas = []
        ...
        return defined_tables + map(
            lambda y: self.parse_relation_alias(str(y)), without_commas)

    def parse_relation_alias(self, query):
        # parse the 'as' part of query
        parsed = re.split("\W+(as|AS|As|aS)\W+",
                          query)  # can't set re.ignorecase!!
        return (None if len(parsed) < 3 else parsed[len(parsed) - 1], query
                if len(parsed) < 3 else parsed[0:len(parsed) - 2][0], query)
