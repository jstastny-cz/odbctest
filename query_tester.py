...
class QueryTester(object):
    ...
    def __init__(self, db_connection, query_dir, results_dir, scenario_name,
                 expected_dir):
    ...
    def query_files(self, filename=None, query_files=[]):
        full_file_name = join(self.query_dir,
                              filename) if filename else self.query_dir
        if isfile(full_file_name):
            query_files.append(filename)
        elif isdir(full_file_name):
            for f in listdir(full_file_name):
                self.query_files(
                    join(filename, f) if filename else f, query_files)
        ...
        for filename in self.query_files(filename):
            ...
            for query_tuple in reader.read(self.query_dir + "/" + filename):
                try:
                    ...
                    results_container = runner.run(*query_tuple)
                    result = result and (results_container is not None)
                    ResultsWriter(results_container).export(
                        result_dirname + "/" + query_set_name + "_" +
                        query_tuple[0] + ".xml")
                finally:
                    if runner is not None: runner.destroy()
        return result
        ...
        for filename in self.query_files(filename_given):
            ...
            for query_tuple in reader.read(self.query_dir + "/" + filename):
                ...
                try:
                    ...
                    actual_results = runner.run(*query_tuple)
                    comp_result = ResultsComparator().compare(
                        expected_results, actual_results)
                    if not comp_result.success:
                        num_errors += 1
                        self.report_failures(query_set_name, expected_results,
                                             actual_results, comp_result)
                finally:
                    if runner is not None: runner.destroy()
    ...
    def report_failures(self, query_set_name, expected_results, actual_results,
                        comparation_results):
        writer = ResultsWriter(actual_results, expected_results,
                               comparation_results)
