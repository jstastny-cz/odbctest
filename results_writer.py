...
class ResultsWriter:
    ...
    def write_generated_results(self):
        ...
        if self.exception_message:
            self.write_exception("queryResults", "exception",
    ...
    def write_comparation_results(self, comparation_results):
        ...
        for failure in comparation_results.failures:
            etree.SubElement(
                self.xml.find("queryResults"), "failureMessage"
            ).text = failure.message + " (Expected:" + unicode(
                failure.expected) + "; Actual:" + unicode(
                    failure.actual) + ";)"
        if self.exception_message:
            self.write_exception("queryResults", "actualException",
                                 self.exception_type, self.exception_message,
                                 self.exception_class)
        else:
            act_el = etree.SubElement(
                self.xml.find("queryResults"), "actualQueryResults")
        ...
        if self.exp_exception_message:
            self.write_exception(
                "queryResults", "expectedException", self.exp_exception_type,
                self.exp_exception_message, self.exp_exception_class)
        else:
            ex_el = etree.SubElement(
                self.xml.find("queryResults"), "expectedQueryResults")
    ...
    def write_query_results_compare_mode(self):
        el_query_results = etree.SubElement(self.xml, "queryResults", {
    ...
    def write_cols(self, parent_element, result_cols):
        ...
        for x in result_cols:
            etree.SubElement(el_select, "dataElement", {
                "type": x.col_type
            }).text = x.name
    ...
    def write_results(self, parent_element, num_rows, num_cols, result_cols,
        ...
        el_table = etree.SubElement(parent_element, "table", {
            "rowCount": str(num_rows),
            "columnCount": str(num_cols)
        })
        for i in range(0, num_rows):
            el_row = etree.SubElement(el_table, "tableRow")
            for j in range(0, num_cols):
                el_cell = etree.SubElement(el_row, "tableCell")
                el_text = etree.SubElement(el_cell,
                                           str(result_cols[j].col_type))
